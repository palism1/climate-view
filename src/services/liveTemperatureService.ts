 ...
 */
<<<<<<< HEAD
export const fetchLiveTemperatureFromNOAA = async (
  latitude: number,
  longitude: number
) => {
  try {
    const metadataResponse = await fetch(
      `${NOAA_BASE_URL}/${latitude},${longitude}`
    );
    ...
    console.log(
      `NOAA forecast data for (${latitude}, ${longitude}):`,
      forecastData
    );
    ...
    return {
      temperature,
      unit: forecastData.properties.periods[0]?.temperatureUnit,
    };
  } catch (error) {
    console.error(
      `Error fetching live temperature data from NOAA for (${latitude}, ${longitude}):`,
      error
    );
    throw error;
  }
 ...
 */
<<<<<<< HEAD
export const fetchLiveTemperatureFromNASA = async (
  latitude: number,
  longitude: number
) => {
  try {
    ...
    console.log(
      `Fetching NASA POWER data for Lat: ${latitude}, Lon: ${longitude}, Date: ${formattedDate}`
    );
